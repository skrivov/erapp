...
export function ArtifactPreview({ url, filename, placeholder, initialScale = 1, maxHeightClass = "max-h-96" }: ArtifactPreviewProps) {
  const clampedInitial = clampScale(initialScale);
  const [scale, setScale] = useState(() => clampedInitial);

  useEffect(() => {
    setScale(clampedInitial);
  }, [clampedInitial, url]);

  if (!url) {
    return (
      <div className="border border-dashed border-slate-200 bg-slate-50 p-6 text-sm text-slate-500">
        {placeholder ?? "Original receipt preview will appear here once available."}
      </div>
    );
  }
...
const handleReset = () => setScale(clampedInitial);
  ...
  return (
    <section className="flex flex-col gap-2 border border-slate-300 bg-white p-4">
      ...
      <div className={`flex-1 overflow-auto border border-slate-200 bg-slate-50 ${maxHeightClass}`}>
